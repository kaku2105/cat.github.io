...
export const LandingPage = withDependencies(
	[named(PageFeatureConfigSymbol, name), LandingPageAPISymbol, pageIdSym, optional(PopupUtilsSymbol)],
	(config, { updateClassNames }, pageId, popupUtils?: IPopupUtils): IPageWillMountHandler => ({
		name: 'landingPage',
		pageWillMount: () => {
			if (pageId === 'masterPage' || popupUtils?.isPopupPage(pageId)) {
