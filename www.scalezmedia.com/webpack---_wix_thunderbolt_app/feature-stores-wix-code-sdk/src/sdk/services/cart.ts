...
export class CartServiceSdk extends WixStoresServiceSdk {
	async getCurrentCart(): Promise<any> {
		...
		const url = (this.platformUtils.essentials.experiments as any).experiments['specs.stores.useFullUrlForVelo']
			? this.getAbsoluteUrl(CART_API_BASE_URL)
			: CART_API_BASE_URL
		const cartApiFactory = WixEcommerceCartServicesWeb(url).Carts()
