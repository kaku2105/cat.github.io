...
import { createFedopsLogger, captchaChallengeActions } from '@wix/thunderbolt-commons'
...
export async function SiteMembersSdkFactory({
	...
	const MemberDetailsOnPreviewMode =
		isEditMode && platformEnvData.site.experiments['specs.thunderbolt.MemberDetailsOnPreviewMode']
	const {
		window: { isSSR },
	...
	const memberDetails = MemberDetailsOnPreviewMode
	...
	const _login: LegacySiteMembersWixCodeSdkWixCodeApi['login'] = async (email, password, options) => {
		...
		if (options?.recaptchaToken) {
			...
			await withCaptchaChallengeHandler(captchaChallengeActions.login, (recaptchaToken) =>
	...
	const _register: IRegister = (isMembersApi: boolean) => async (
		...
		try {
			const data = options?.recaptchaToken
				...
				: await withCaptchaChallengeHandler(captchaChallengeActions.signup, (recaptchaToken) =>
