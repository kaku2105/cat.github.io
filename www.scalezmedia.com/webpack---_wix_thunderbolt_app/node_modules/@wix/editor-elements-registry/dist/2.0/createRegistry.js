...
async function loadContext({
    ...
    libraries.forEach((library, index) => {
        if (exports.isLibraryTopology(library)) {
            const manifestName = editor_elements_conventions_1.getManifestName({
...
function createRegistry({
    ...
    return async (params) => {
        ...
        const resourceLoader = createResourceLoader_1.createResourceLoader({
            options,
        ...
        if (registryOverrideLibrary) {
            const registry = await loadExternalRegistry_1.loadExternalRegistry({
                resourceLoader,
                host,
        ...
        async function createComponentLoaders(manifests) {
            const tasks = manifests.map(async ({
                ...
                // without *.local
                const namespace = editor_elements_conventions_1.withProdNamespace(manifest.getNamespace());
                const mode = (params.modes && params.modes[namespace]) || params.mode || 'lazy';
                // @ts-expect-error
                librariesComponents[order] =
                    mode === 'lazy' ?
                    createComponentLoaders_1.createLazyComponentLoaders({
                        manifest,
                        loadBundle,
                    }) :
                    await createComponentLoaders_1.createEagerComponentLoaders({
                        manifest,
                        loadBundle,
                    });
            });
        ...
        return {
            ...
            getHostBundleAssets() {
                return utils_1.flat(context.manifests.map(({
                    manifest
                }) => {
