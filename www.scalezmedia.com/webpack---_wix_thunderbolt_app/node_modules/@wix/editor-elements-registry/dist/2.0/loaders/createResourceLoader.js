...
function createResourceLoader({
    ...
    const loadResource = ({
        ...
        times: RETRY_TIMES,
        task: async () => {
            if (options && options.useScriptsInsteadOfEval && utils_1.isBrowser()) {
    ...
    return {
        ...
        createBundleLoader({
            ...
            return async ({
                 ...
                 */
                if (assets) {
                    loadAssets_1.loadAssets(assets.filter(a => !!a));
                }
                if (id && (utils_1.isBrowser() || utils_1.isWorker())) {
                    const model = await evaluateBundle_1.getModel(id);
                    if (model) {
                        return model;
                    }
                }
