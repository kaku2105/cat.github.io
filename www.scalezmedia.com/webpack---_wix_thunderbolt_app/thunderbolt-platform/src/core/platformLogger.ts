...
import { create, IAppIdentifier } from '@wix/fe-essentials-viewer-platform/fedops'
...
export const PlatformLogger = (
	...
	const withReportingAndErrorHandling: IPlatformLogger['withReportingAndErrorHandling'] = async (phase, asyncMethod, params) => {
		...
		const removeUnfinishedTask = unfinishedTasks.add(`${phase}_${appDefinitionId}${controllerType ? `_${controllerType}` : ''}`)
