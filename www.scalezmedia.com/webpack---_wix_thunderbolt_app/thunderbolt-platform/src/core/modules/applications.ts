...
import { getDisplayedId, getFullItemId } from '@wix/thunderbolt-commons'
...
import type { ControllerData, IControllersExports, IViewerHandlers } from '../types'
...
import type { ICommonConfigModule } from './commonConfig'
...
const Applications = (
	...
	const runApplication = async (appDefinitionId: string) => {
		...
		const wixCodeApi = await logger.runAsyncAndReport('init_wix_code_apis', () => wixCodeApiFactory.initWixCodeApiForApplication(appDefinitionId, platformAppServicesApi.essentials))
